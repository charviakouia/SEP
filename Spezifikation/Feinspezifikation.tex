...
%\newcommand{\disambiguationrule}{\arrayrulecolor{lightgray}\midrule\arrayrulecolor{black}}
...
\renewenvironment{controls}
{
    \begin{table}[H]
        ...
        \begin{tabular}{ p{7em} p{25em} p{7em} }
            \toprule
            \textbf{Tag} & \textbf{Beschreibung} & \textbf{Sichtbarkeit}\\
            \midrule
        }
        {
            \bottomrule
...
Jedes \tag{f}{viewParam}-Element ist in ein \tag{f}{metadata}-Element eingebettet.
...
\begincontrols[header]
    \tag{h}{commandButton} & kontextsensitive Hilfe anzeigen & \M{action} & \B{header.displayHelpText()} & \\ % QUESTION can that work??
...
\begincontrols[public/email-confirmation]
    \3{\tag{f}{viewParam}} & \3{Kennzeichen der E-Mail-Bestätigung} & \M{name} & \M{token} & \3{\PUB}\\
...
% TASK rename to Kategorienerstellung / category creator
\subsubsection{Kategorienbearbeitung}\label{page_category_editor}

\begincontrols[staff/category-editor]
    \INP & Kategorienname & \M{value} & \B{categoryEditor.category.name} & \BIB\\
    \TXT & Kategorienbeschreibung & \M{value} & \B{categoryEditor.category.description} & \BIB\\
    \BTN & Kategorie erstellen & \M{action} & \B{categoryEditor.save()} & \BIB\\ % save() -> create()
\endcontrols
...
\begincontrols[public/category-browser]
    ...
    % TASK rename category-editor to category-creator
    \tag{h}{commandLink} & Kategorie unter der ausgewählten Kategorie erstellen & \M{action} & \hyperref[page_category_editor]{\M{staff/category-editor.xhtml}} & \BIB\\
    \BTN & ausgewählte Kategorie löschen & \M{action} & \B{categoryBroswer.delete()} & \BIB\\ % TASK param categ id
...
\begincontrols[staff/medium-creation]
    \2{\tag{ui}{repeat}} & \2{Medienattribute} & \M{value} & \B{mediumCreation.medium.attributes} & \2{\BIB}\\
    & & \M{var} & \hyperref[subcontrol_medium_creation_attribute]{\M{attribute}} &\\
    \disambiguationrule
    \INP & Medienkategorie & \M{value} & \B{mediumCreation.medium.category} & \BIB\\
    \INP & Rückgabefrist & \M{value} & \B{mediumCreation.medium.returnPeriod} & \BIB\\
    \INP & Standort des ersten Exemplars & \M{value} & \B{mediumCreation.copy.location} & \BIB\\
    \INP & Signatur des ersten Exemplars & \M{value} & \B{mediumCreation.copy.signature} & \BIB\\
    \BTN & Medium und dessen erstes Exemplar erstellen & \M{action} & \B{mediumCreation.create()} & \BIB\\
\endcontrols

\beginsubcontrols[Attribut][subcontrol_medium_creation_attribute]
...
\begincontrols[public/medium]
    \3{\tag{f}{viewParam}} & \3{Kennzeichen des Mediums} & \M{name} & \M{id} & \3{\PUB}\\
...
\begincontrols[public/password-reset]
    \3{\tag{f}{viewParam}} & \3{Kennzeichen der Passwortzurücksetzung} & \M{name} & \M{token} & \3{\PUB}\\
...
\begincontrols[account/profile]
    % welche sichtbarkeit
    \3{\tag{f}{viewParam}} & \3{Kennzeichen des Nutzers} & \M{name} & \M{id} & \3{\USR}\\
    ...
    \LNK & zur den abzuholenden Exemplaren & \M{value} & \hyperref[page_my_copies_ready_for_pickup]{ \M{account/my-copies-ready-for-pickup.xhtml}} & \USR\\
    \LNK & zu den ausgeliehenen Exemplaren & \M{value} & \hyperref[page_my_borrowed_copies]{ \M{account/my-borrowed-copies.xhtml}} & \USR\\
