...
%\DeclareUnicodeCharacter{2BAC}{\parentarrow}
...
\begincontrols[public/login]
    \OUT & & \M{rendered} & \B{session != null} & \\ % TASK note that you are already logged in
    \FRM & & \M{rendered} & \B{session == null} & \\
    \INDENT\INP & E-Mail-Adresse & \M{value} & \B{login.user.emailAddress} & \ANO\\
    \INDENT\PAS & Passwort & \M{value} & \B{login.password} & \ANO\\ % BEACON password converter to hash
    \INDENT\BTN & anmelden & \M{action} & \B{login.logIn()} & \ANO\\
    \INDENT\tag{h}{commandLink} & Passwort zurücksetzen & \M{action} & \B{login.resetPassword()} & \ANO\\
\endcontrols
