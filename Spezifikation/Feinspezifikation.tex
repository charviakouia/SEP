...
\long\def\begincontrols [#1] #2\endcontrols
{
    \begin{table}[H]
        ...
        \begin{tabular}{ l l l l l }
            ...
            \cmidrule(r){3-4}
            \textbf{Tag} & \textbf{Beschreibung} & \textbf{Name} & \textbf{Bindung} & \textbf{Sichtbarkeit}\\
            \midrule
            #2
            \bottomrule
        \end{tabular}
...
\begincontrols[header]
    ...
    % BEACON TASK + rendered=$
    \OUT & kontextsensitive Hilfe & \M{value} & BUNDLE-HELP & \\
    \INP & Mediensuchterm & \M{value} & \B{header.mediumSearchTerm} & \\ % BEACON store in sep DTO
    ...
    \tag{h}{commandLink} & abmelden & \M{action} & \B{header.logOut()} & \\
    \LNK & zur Anmeldemaske & \M{value} & \hyperref[page_login]{\M{public/login.xhtml}} & \\
    \LNK & zur Registrierungsseite & \M{value} & \hyperref[page_registration]{\M{public/registration.xhtml}} & \\
    \LNK & zur Profilseite & \M{value} & \hyperref[page_profile]{\M{account/profile.xhtml?id=\RB{header.user.id}}} & \\
    \LNK & zur Direktausleihe & \M{value} & \hyperref[page_direct_lending]{\M{staff/direct-lending.xhtml}} & \BIB\\
    \LNK & zur Medienrückgabe & \M{value} & \hyperref[page_return_form]{\M{staff/return-form.xhtml}} & \BIB\\
    \LNK & zu den abzuholenden Exemplaren aller Nutzer & \M{value} & \hyperref[page_copies_ready_for_pickup_all_users]{\M{staff/copies-ready-for-pickup-all-users.xhtml}} & \BIB\\
    \LNK & zur Medienerstellung & \M{value} & \hyperref[page_medium_creator]{\M{staff/medium-creator.xhtml}} & \BIB\\
\endcontrols
...
\begincontrols[account/copies-ready-for-pickup]
    \3{\PRM} & \3{Kennzeichen des Nutzers} & \M{name} & \M{id} & \3{\PUB}\\
...
\begincontrols[public/login]
    \INP & E-Mail-Adresse & \M{value} & \B{login.user.emailAddress} & \ANO\\
    \PAS & Passwort & \M{value} & \B{login.password} & \ANO\\ % BEACON password converter to hash
    \BTN & anmelden & \M{action} & \B{login.logIn()} & \ANO\\
    \tag{h}{commandLink} & Passwort zurücksetzen & \M{action} & \B{login.resetPassword()} & \ANO\\
\endcontrols
...
\begincontrols[public/privacy-policy]
    ...
    \TXT & Datenschutzerklärung & \M{value} & \B{privacyPolicy.application.privacyPolicy} & \ADM\\
    \BTN & Änderungen speichern & \M{action} & \B{privacyPolicy.save()} & \ADM\\
\endcontrols

\subsubsection{Direktausleihe}\label{page_direct_lending}

\begincontrols[staff/direct-lending]
    \tag{bibi}{reactiveInputText} & E-Mail-Adresse des Kunden & \M{value} & \B{directLending.user.emailAddress} & \BIB\\
    \disambiguationrule
    \2{\tag{ui}{repeat}} & \2{Signaturen der auszuleihenden Exemplare} & \M{value} & \B{directLending.copies} & \2{\BIB}\\
    & & \M{var} & \hyperref[subcontrol_direct_lending_copy]{\M{copy}} & \\
    \disambiguationrule
    \BTN & weiteres Signatureingabefeld hinzufügen & \M{action} & \B{directLending.addSignatureInputField()} & \BIB\\
    \BTN & Exemplare verleihen & \M{action} & \B{directLending.lendCopies()} & \BIB\\
\endcontrols

\beginsubcontrols[Exemplar (5 bei Seitenaufruf)][subcontrol_direct_lending_copy]
\tag{bibi}{reactiveInputText} & Signatur des Exemplars & \M{value} & \B{copy.signature} & \BIB\\
\endsubcontrols

\subsubsection{E-Mail-Bestätigung}\label{page_email_confirmation}

\begincontrols[public/email-confirmation]
    \3{\PRM} & \3{Kennzeichen der E-Mail-Bestätigung} & \M{name} & \M{token} & \3{\PUB}\\
...
\begincontrols[public/site-notice]
    ...
    \TXT & Impressum & \M{value} & \B{siteNotice.application.siteNotice} & \ADM\\
    \BTN & Änderungen speichern & \M{action} & \B{siteNotice.save()} & \ADM\\
\endcontrols

\subsubsection{Kategorienerstellung}\label{page_category_creator}

\begincontrols[staff/category-creator]
    \INP & Kategorienname & \M{value} & \B{categoryCreator.category.name} & \BIB\\
    \TXT & Kategorienbeschreibung & \M{value} & \B{categoryCreator.category.description} & \BIB\\
    \BTN & Kategorie erstellen & \M{action} & \B{categoryCreator.createCategory()} & \BIB\\
\endcontrols

\subsubsection{Kategorierenstöberer}\label{page_category_browser}

\begincontrols[public/category-browser]
    \tag{bibi}{reactiveInputField} & Kategoriensuchterm & \M{value} & \B{categoryBrowser.searchTerm} & \PUB\\ % BEACON sep DTO? SearchDTO
    \BTN & Kategorien suchen & \M{action} & \B{categoryBroswer.searchCategory()} & \PUB\\
    \tag{bibi}{treeView} & Baumansicht von Kategorien & & & \PUB \\
    \INP & Name der ausgewählten Kategorie & \M{value} & \B{categoryBrowser.category.name} & \BIB\\ % TASK cond readonly
    \TXT & Beschreibung der ausgewählten Kategorie & \M{value} & \B{categoryBrowser.category.description} & \BIB\\ % TASK cond readonly
    \tag{h}{commandLink} & Kategorie unter der ausgewählten Kategorie erstellen & \M{action} & \hyperref[page_category_creator]{\M{staff/category-creator.xhtml}} & \BIB\\
    \BTN & ausgewählte Kategorie löschen & \M{action} & \B{categoryBroswer.deleteCategory()} & \BIB\\ % TASK param categ id
    \LST & Medienvorschauen in der ausgewählten Kategorie & & & \PUB\\ % TASK sep tex-table
\endcontrols
...
\begincontrols[public/contact]
    % TASK readonly for users and staff
    \TXT & Kontaktinformationen & \M{value} & \B{contact.application.contactInfo} & \ADM\\
    \BTN & Änderungen speichern & \M{action} & \B{contact.save()} & \ADM\\
\endcontrols
...
\begincontrols[staff/medium-creator]
    \2{\tag{ui}{repeat}} & \2{Medienattribute} & \M{value} & \B{mediumCreator.medium.attributes} & \2{\BIB}\\
    & & \M{var} & \hyperref[subcontrol_medium_creator_attribute]{\M{attribute}} &\\
    \disambiguationrule
    \INP & Medienkategorie & \M{value} & \B{mediumCreator.medium.category} & \BIB\\
    \INP & Rückgabefrist & \M{value} & \B{mediumCreator.medium.returnPeriod} & \BIB\\
    \INP & Standort des ersten Exemplars & \M{value} & \B{mediumCreator.copy.location} & \BIB\\
    \INP & Signatur des ersten Exemplars & \M{value} & \B{mediumCreator.copy.signature} & \BIB\\
    \BTN & Medium und dessen erstes Exemplar erstellen & \M{action} & \B{mediumCreator.createMediumAndFirstCopy()} & \BIB\\
\endcontrols

\beginsubcontrols[Attribut][subcontrol_medium_creator_attribute]
% QUESTION what other attributes (lavel attribut.type??)
\INP & Attributwert & \M{value} & \B{attribute.value} & \BIB\\
\endsubcontrols

\subsubsection{Medienrückgabe}\label{page_return_form}

\begincontrols[staff/return-form]
    \tag{bibi}{reactiveInputField} & E-Mail-Adresse des Kunden & \M{value} & \B{returnForm.user.emailAddress} & \BIB\\
    \disambiguationrule
    \2{\tag{ui}{repeat}} & \2{Signaturen der abzugebenden Exemplare} & \M{value} & \B{returnForm.copies} & \2{\BIB}\\
    & & \M{var} & \hyperref[subcontrol_return_form_copy]{\M{copy}} & \\
    \disambiguationrule
    \BTN & weiteres Signatureingabefeld hinzufügen & \M{action} & \B{returnForm.addSignatureInputField()} & \BIB\\
    \BTN & Exemplare abgeben & \M{action} & \B{returnForm.returnMedium()} & \BIB\\
\endcontrols

\beginsubcontrols[Exemplar (5 bei Seitenaufruf)][subcontrol_return_form_copy]
\tag{bibi}{reactiveInputField} & Signatur des Exemplars & \M{value} & \B{copy.signature} & \BIB\\
\endsubcontrols
...
\begincontrols[public/medium-search]
    \INP & freier Suchterm & \M{value} & \B{mediumSearch.generalSearchTerm} & \PUB\\
    \BTN & Medien suchen & \M{action} & \B{mediumSearch.search()} & \PUB\\
    \tag{ui}{repeat} & differenzierte Suchanfragen & \M{value} & \B{mediumSearch.nuancedSearchQueries} & \PUB\\
    \BTN & weitere differenzierte Suchanfrage hinzufügen & \M{action} & \B{mediumSearch.addNuancedSearchQuery())} & \PUB\\
    % BEACON TEMPORARY
    \LST & aller Suchergebnisse (als Medienvorschauen) & & & \PUB\\ % TASK separate tex-table
\endcontrols

\beginsubcontrols[differenzierter Suchabfrage (3 bei Seitenaufruf)][subcontrol_medium_search_query]
    \DRP & Suchoperator & \M{value} & \B{query.operator} & \PUB\\ % TASK converter
    \DRP & Suchkriterium & \M{value} & \B{query.criterion} & \PUB\\ % TASK converter
    \INP & Suchterm & \M{value} & \B{query.term} & \PUB\\
\endsubcontrols

\subsubsection{Mediumsansicht}\label{page_medium}

\begincontrols[public/medium]
    \3{\PRM} & \3{Kennzeichen des Mediums} & \M{name} & \M{id} & \3{\PUB}\\
    & & \M{value} & \B{medium.medium.id} & \\
    & & \M{required} & \M{true} & \\
    \disambiguationrule
    % TODO nur sichtbar wenn man von der Suche kommt
    \LNK & zurück zur Trefferliste & \M{value} & \hyperref[page_medium_search]{\M{public/medium-search.xhtml}} & \PUB\\
    \disambiguationrule
    \2{\tag{ui}{repeat}} & \2{Medienattribute} & \M{value} & \B{medium.medium.attributes} & \2{\PUB}\\
    & & \M{var} & \hyperref[subcontrol_medium_attribute]{\M{attribute}} & \\
    \disambiguationrule
    \BTN & Änderung an den Attributen speichern & \M{action} & \B{medium.saveAttributes()} & \BIB\\
    \BTN & willkürliches Exemplar abholen & \M{action} & \B{medium.pickUpAnyCopy()} & \USR\\
    \OUT & Rückgabefrist unter Einbeziehung der nutzerbezogenen & \M{value} & \B{medium.returnPeriod} & \USR\\
    \INP & mediumsspezifische Rückgabefrist & \M{value} & \B{medium.medium.returnPeriod} & \BIB\\
    \BTN & Änderungen an der Rückgabefrist speichern & \M{action} & \B{medium.saveReturnPeriod()} & \BIB\\
    \BTN & Medium löschen & \M{action} & \B{medium.delete()} & \BIB\\
    \disambiguationrule
    \3{\tag{h}{dataTable}} & \3{Exemplare des Mediums} & \M{value} & \B{medium.copies} & \3{\PUB}\\
    & & \M{var} & \hyperref[data_table_medium_copies]{\M{copy}} & \\
    & & \M{varStatus} & \M{status} & \\
    \disambiguationrule
    \tag{bibi}{reactiveInputField} & Standort eines neuen Exemplars & \M{value} & \B{medium.copy.location} & \BIB\\
    \INP & Signatur eines neuen Exemplars & \M{value} & \B{medium.copy.signature} & \BIB\\
    \BTN & neues Exemplar erstellen & \M{action} & \B{medium.createCopy()} & \BIB\\
\endcontrols
...
\begincontrols[admin/medium-schema-editor]
    \2{\tag{ui}{repeat}} & \2{Medienattribute} & \M{value} & \B{mediumSchemaEditor.attributes} & \2{\ADM}\\
    & & \M{var} & \hyperref[subcontrol_schema_editor_attribute]{\M{attribute}} & \\
    \disambiguationrule
    \BTN & weiteres Attributeingabefeld hinzufügen & \M{action} & \B{mediumSchemaEditor.addAttributeInputField()} & \ADM\\
    \BTN & Änderungen speichern & \M{action} & \B{mediumSchemaEditor.save()} & \ADM\\
\endcontrols

\beginsubcontrols[Attribut][subcontrol_schema_editor_attribute]
    \INP & Attributname &  \M{value} & \B{attribute.name} & \ADM\\
    \DRP & Attributtyp & \M{value} & \B{attribute.type} & \ADM\\
    \DRP & Multiplizität des Attributs & \M{value} & \B{attribute.multiplicity} & \ADM\\
    \DRP & Position des Attributs in der Medienvorschau & \M{value} & \B{attribute.position} & \ADM\\
    \BTN & Attribut löschen & \M{action} & \B{mediumSchemaEditor.deleteAttribute(attribute.id)} & \ADM\\
\endsubcontrols

\subsubsection{Nutzersuche}\label{page_user_search}

\begincontrols[admin/user-search]
    \INP & Nutzersuchterm & \M{value} & \B{userSearch.searchTerm} & \ADM\\ % UserSearchDTO?
    \BTN & Nutzer suchen & \M{action} & \B{userSearch.searchUser()} & \ADM\\
    \CHK & Einschränkung auf ge-/entsperrte Nutzer & \M{value} & \B{userSearch.accountStatus} & \ADM\\
    \LST & Suchergebnisse & & & \ADM\\ % TASK sep tex table
\endcontrols

\subsubsection{Passwortzurücksetzung}\label{page_password_reset}

\begincontrols[public/password-reset]
    \3{\PRM} & \3{Kennzeichen der Passwortzurücksetzung} & \M{name} & \M{token} & \3{\PUB}\\
    & & \M{value} & \B{passwordReset.token} & \\
    & & \M{required} & \M{true} & \\
    \disambiguationrule
    \PAS & neues Passwort & \M{value} & \B{passwordReset.password} & \PUB\\
    \PAS & bestätigtes neues Passwort & \M{value} & \B{passwordReset.confirmedPassword} & \PUB\\
    \BTN & Passwort zurücksetzen & \M{action} & \B{passwordReset.resetPassword()} & \PUB\\
\endcontrols
...
\begincontrols[account/profile]
    % welche sichtbarkeit
    \3{\PRM} & \3{Kennzeichen des Nutzers} & \M{name} & \M{id} & \3{\USR}\\
    & & \M{value} & \B{profile.user.id} &\\
    & & \M{required} & \M{true} & \\
    \disambiguationrule
    \INP & Vorname & \M{value} & \B{profile.user.firstName} & \USR\\
    \INP & Nachname & \M{value} & \B{profile.user.lastName} & \USR\\
    \PAS & Passwort & \M{value} & \B{profile.password} & \USR\\
    \PAS & bestätigtes Passwort & \M{value} & \B{profile.confirmedPassword} & \USR\\
    \INP & E-Mail-Adresse & \M{value} & \B{profile.user.emailAddress} & \USR\\
    \INP & Postleitzahl des Wohnsitzes & \M{value} & \B{profile.user.zipCode} & \USR\\
    \INP & Stadt des Wohnsitzes & \M{value} & \B{profile.user.city} & \USR\\
    \INP & Straße des Wohnsitzes & \M{value} & \B{profile.user.street} & \USR\\
    \INP & Hausnummer des Wohnsitzes & \M{value} & \B{profile.user.streetNumber} & \USR\\
    \DRP & Nutzerrolle & \M{value} & \B{profile.user.role} & \ADM\\
    \CHK & Accountstatus (ent-/gesperrt) & \M{value} & \B{profile.user.accountStatus} & \ADM\\
    \INP & Rückgabefrist & \M{value} & \B{profile.user.returnPeriod} & \ADM\\ % TASK UPDATE dto from lendingPeriod -> returnPeriod
    \BTN & Änderungen am Profil speichern & \M{action} & \B{profile.save()} & \USR\\
    \LNK & zur den abzuholenden Exemplaren & \M{value} & \hyperref[page_copies_ready_for_pickup]{ \M{account/my-copies-ready-for-pickup.xhtml?id=\RB{profile.user.id}}} & \USR\\
    \LNK & zu den ausgeliehenen Exemplaren & \M{value} & \hyperref[page_borrowed_copies]{ \M{account/my-borrowed-copies.xhtml?id=\RB{profile.user.id}}} & \USR\\
    \BTN & Account schließen & \M{action} & \B{profile.delete()} & \USR\\ % slash admin...
\endcontrols

\subsubsection{Registrierungsseite}\label{page_registration}

\begincontrols[public/registration]
    \INP & Vorname & \M{value} & \B{registration.user.firstName} & \ANO/\ADM\\
    \INP & Nachname & \M{value} & \B{registration.user.lastName} & \ANO/\ADM\\
    \PAS & Passwort & \M{value} & \B{registration.password} & \ANO/\ADM\\
    \PAS & bestätigtes Passwort & \M{value} & \B{registration.confirmedPassword} & \ANO/\ADM\\
    \INP & E-Mail-Adresse & \M{value} & \B{registration.user.emailAddress} & \ANO/\ADM\\
    \INP & Postleitzahl des Wohnsitzes & \M{value} & \B{registration.user.zipCode} & \ANO/\ADM\\
    \INP & Stadt des Wohnsitzes & \M{value} & \B{registration.user.city} & \ANO/\ADM\\
    \INP & Straße des Wohnsitzes & \M{value} & \B{registration.user.street} & \ANO/\ADM\\
    \INP & Hausnummer des Wohnsitzes & \M{value} & \B{registration.user.streetNumber} & \ANO/\ADM\\
    \DRP & Nutzerrolle & \M{value} & \B{registration.user.role} & \ADM\\
    \BTN & Account registrieren & \M{action} & \B{registration.register()} & \ANO/\ADM\\
\endcontrols

\subsubsection{Verwaltungsseite}\label{page_administration}

\begincontrols[admin/administration]
    \INP & systemweite Rückgabefrist & \M{value} & \B{administration.application.returnPeriod} & \ADM\\
    \INP & Mahnungsfrist & \M{value} & \B{administration.application.warningPeriod} & \ADM\\
    \INP & Abholungsfrist & \M{value} & \B{administration.application.pickupPeriod} & \ADM\\
    \INP & Name des Systems & \M{value} & \B{administration.application.name} & \ADM\\
    \tag{h}{inputFile} & Logo des Systems & \M{value} & \B{administration.application.logo} & \ADM\\
    \CHK & Registrierungsstatus (offen/geschlossen) & \M{value} & \B{administration.application.registerStatus} & \ADM\\
    \INP & erlaubte E-Mail-Adressen-Suffixe & \M{value} & \B{administration.application.emailAddressSuffixRegEx} & \ADM\\
    \DRP & Farbschema & \M{value} & \B{administration.application.theme} & \ADM\\
    \BTN & Änderungen speichern & \M{action} & \B{administration.save()} & \ADM\\
    \LNK & zur Registrierungsseite zur Nutzererstellung & \M{value} & \hyperref[page_registration]{\M{public/registration.xhtml}} & \ADM\\
    \LNK & zum Mediumsschema-Editor & \M{value} & \hyperref[page_medium_schema_editor]{\M{admin/medium-schema-editor.xhtml}} & \ADM\\
    \LNK & zu den Leihfristverstößen & \M{value} & \hyperref[page_lending_period_violations]{\M{admin/lending-period-violations.xhtml}} & \ADM\\
    \LNK & zu der Nutzersuche & \M{value} & \hyperref[page_user_search]{\M{admin/user-search.xhtml}}& \ADM\\
    \INP & Nutzersuchterm & \M{value} & \B{administration.userSearchTerm} & \ADM\\
    \BTN & Nutzer suchen & \M{action} & \B{administration.searchUser()} & \ADM\\
\endcontrols
