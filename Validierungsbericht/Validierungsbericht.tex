...
Aus dem Facelet-Code unserer Anwendung wird vom JSF-Framework beim Endnutzer anzeigbarer HTML-Code generiert. Dieser wurde im Zuge der Validierung mithilfe des \hyperlink{https://validator.w3.org/}{W3-Validators} auf Fehler und Stilbrüche geprüft. Da unterschiedliche Browser verschieden tolerant gegenüber fehlerhaftem HTML-Code sind, resultierten diese Fehler nicht zwingend in falschen Anzeigen und wurden so beim Implementieren übersehen. Einige Warnungen, die auf `bad-practices' Hinweisen sollen, sind JSF-internen Prozessen beim Generieren des Outputs geschuldet und außerhalb unserer Kontrolle. Darunter fallen unter anderem:
\begin{itemize}
\item Generieren des obsoleten Attributes \texttt{type=''text/javascript''} innerhalb des HTML-Elements \texttt{<script/>}.
\item Setzen des Attributes \texttt{name} auf den gleichen Wert des im Facelet-Code angegebenen und ebenfalls im HTML-Code präsenten Attributes \textt{id} in verschiedensten HTML-Elementen.
\end{itemize}
Es folgt eine kurze, unvollständige Auflistung der wichtigsten gefundenen (und verbesserten) Fehler im Zuge der Validierung:
\begin{itemize}
\item `div inside span'-Error, aufgetreten durch ein fehlendes Attribut \texttt{layout=''block''} in Facelet-Elementen vom Typ \texttt{h:panelGroup}.
\item Fehlende Deklaration des Attributes \texttt{lang} im führenden \texttt{html} Start-Tag.
\end{itemize}
