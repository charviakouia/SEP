...
Durch eine paarweise aufsteigende Verkettung, erhält man somit die folgende Liste: Anonymen Nutzer, angemeldete Nutzer, Bibliothekmitarbeiter, und Administratoren.

Zudem wird im Folgenden unter 'editieren' auch das Erstellen und Löschen gemeint, wenn nicht anders spezifiziert.
	...
	\subsection{Nutzerverwaltung}
		\subsubsection{Administrator}
			...
			\specification{F}{30}{Ein Nutzerprofil kann editiert werden. Es lassen sich dadurch alle Nutzerdaten und die Rolle aller Nutzer verändern. 
				...
				In diesem Fall wird eine Bestätigung gefordert und sämtliche Exemplare und leer-gewordene Medien mit gelöscht. }
			\specification{F}{60}{Es kann mit Angabe von Nutzerattributen nach Nutzern gesucht werden. Es können auch alle Nutzer im System gezeigt werden. }
		...
		\subsubsection{Angemeldete Nutzer}
			...
			\specification{F}{120}{Das eigene Profil kann editiert werden. Dabei ist es nicht möglich, das Konto zu löschen, wenn die Rückgabe eines Exemplars erwartet wird.
				Um das Passwort zurückzusetzen muss der Nutzer dazu einen per E-Mail zugeschickten Link verwenden.
				Die Verifikation einer neuen E-Mail Adresse erfolgt analog zu der Registrierung. }
	\subsection{Navigation \& Suche}
		\subsubsection{Alle Nutzer}
			...
			\specification{F}{180}{Mit Eingabe von Kategorie und jeweiligen Attributen kann eine Suche durchgeführt werden. }
	...
	\subsection{Ausleihe \& Rückgabe}
		\subsubsection{Administrator}
			...
			\specification{W}{290}{Die maximal Reservierungsdauer ist setzbar. Dies bezeichnet die Zeitdauer, in der ein zuvor nicht-verfügbares Exemplar bei der Wiederverfügbarkeit zur Abholung markiert ist. 
				Insbesondere ist diese Statusmarkierung nur den Nutzern sichtbar, die vor der Wiederverfügbarkeit das Exemplar reserviert haben. 
				Wenn diese Zeit abläuft, ohne dass das gegebene Exemplar im System als ausgeliehen markiert wurde, steht es sofort wieder anderen Nutzern zu Verfügung. }
		\subsubsection{Bibliothekmitarbeiter}
			\specification{F}{300}{Eine Liste von Exemplaren und Nutzer-E-Mails, für die der angegebene Nutzer das gegebene Exemplar zur Abholung markiert hat, ist abrufbar. 
				Dabei wird ein reserviertes Exemplar mehrfach für jeden Reservierenden angezeigt. }
			...
			\specification{F}{322}{Sämtliche Abholungsmarkierungen und Reservierungen können storniert werden. Darüber werden die betroffenen Nutzer per E-Mail benachrichtigt. }
		\subsubsection{Angemeldete Nutzer}
			...
			\specification{W}{340}{Wenn ein Exemplar nicht vorhanden ist, kann es reserviert werden.
				Dadurch wird der Nutzer zu einer Warteliste von Nutzern hinzugefügt, die jeweils per E-Mail benachrichtigt werden, sobald das Exemplar wieder verfügbar ist. (\hyperlink{spec:F:290}{Siehe /F290/}) }
	...
	\subsection{Katalogführung}
		\subsubsection{Administrator}
			\specification{F}{380}{Es kann ein Mediumsschema definiert und geloscht werden. Dabei wird der Attributsatz eines Mediums, der nach diesem Mediumsschema erzeugt wird, festgelegt. 
				Vor dem Löschen eines Schemas wird geprüft, ob Medien, die dem Mediumsschema folgen, existieren. 
				Falls es solche Medien gibt, wird eine Bestätigung gefordert. Anschließend werden alle nach dem gegebenen Mediumsschema erzeugte Medien und entsprechende Exemplare gelöscht. }
		\subsubsection{Bibliothekmitarbeiter}
			...
			\specification{F}{400}{Es kann ein Medium editiert werden. Bei der Erstellung muss ein Mediumsschema gewählt werden und es müssen die entsprechenden Attribute gesetzt werden. 
				Vor dem Löschen eines Mediums wird auf aktuelle Ausleihvorgänge geprüft. Falls ein Exemplar zu der Zeit ausgeliehen ist, wird eine Bestätigung gefordert. 
